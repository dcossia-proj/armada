...
| 🎮 **Dual-screen gaming** | Nested gamescope keeps the top screen's game running full speed while the bottom screen contains a full desktop environment, with a performance overlay. The AYN button toggles the overlay, disabling touch on the bottom screen while showing a full mangoHUD; holding the AYN button disables the bottom OLED panel to save power. When switching to desktop from gamescope, both screens provide a portable, dual-monitor linux setup. |
